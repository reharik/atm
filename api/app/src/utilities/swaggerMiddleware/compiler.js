...
const curriedValidator = require('./curriedValidator');
const mergeDistinct = require('./mergeDistinct');
...
function buildUpParameters(path, verbName) {
  ...
  // merge in or replace parameters = require(verb leve);
...
function compile(document, customValidators) {
  ...
  let compiledPaths = Object.keys(swagger.paths).map(name => {
    ...
    return {
      ...
      expected: (name.match(/[^\/]+/g) || []).map(s => s.toString()) //eslint-disable-line no-useless-escape
    };
  });
